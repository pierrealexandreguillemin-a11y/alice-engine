"""XGBoost Baseline - Entraînement isolé pour comparaison AutoGluon.

RÉUTILISE les modules existants (ISO 5055 - pas de duplication).

Usage:
    python -m scripts.baseline.xgboost_baseline
"""
...
def train_xgboost_baseline(
    ...
    """Entraîne XGBoost baseline isolé - RÉUTILISE scripts/training."""
    import pandas as pd

    logger.info("XGBOOST BASELINE - Reusing scripts/training")

    config = load_hyperparameters(config_path)
    xgboost_params = get_config_value(config, "xgboost", {})
    ...
    result = train_xgboost(X_train, y_train, X_valid, y_valid, xgboost_params)
